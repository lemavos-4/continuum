...
import { ArrowDownTrayIcon, ArrowPathIcon, ArrowUpTrayIcon, ChatBubbleLeftEllipsisIcon, ChevronRightIcon, CodeBracketIcon, BugAntIcon, LifebuoyIcon, LinkIcon, InformationCircleIcon, CurrencyDollarIcon, DocumentTextIcon, ShieldCheckIcon, ClockIcon } from "@heroicons/react/24/outline";
...
function OfflineSyncRow() {
  const { status, pending, syncing } = useOfflineStatus(); const { t } = useLanguage(); const [busy, setBusy] = useState(false); const [lastSync, setLastSync] = useState<number>();
  useEffect(() => { let alive = true; void getLastSyncAt().then((value) => alive && setLastSync(value)); return () => { alive = false; }; }, [pending, syncing]);
  const onSync = async () => { if (!navigator.onLine) { sonnerToast.error(t("profile_offlineToast")); return; } setBusy(true); try { const result = await flushQueue(); if (result.sent === 0 && result.failed === 0) sonnerToast.success(t("profile_upToDate")); else if (result.failed === 0) sonnerToast.success(t("profile_syncComplete", { n: result.sent })); else sonnerToast.warning(t("profile_syncPartial", { sent: result.sent, failed: result.failed })); } finally { setBusy(false); } };
  const active = busy || syncing; const description = status === "offline" ? `${t("profile_workingOffline")}${pending > 0 ? ` · ${t("profile_pending", { n: pending })}` : ""}` : pending > 0 ? t("profile_pending", { n: pending }) : lastSync ? t("profile_lastSync", { time: new Date(lastSync).toLocaleString() }) : t("profile_upToDate");
...
export default function SettingsPage() {
  ...
  return <AppLayout><div className="mx-auto max-w-5xl space-y-7 px-4 py-6 sm:px-6 lg:px-10 lg:py-12">
    ...
    <section className="space-y-3"><SectionTitle eyebrow={t("profile_eyebrowPreferences")} title={t("profile_prefsAppearance")} /><div className="divide-y divide-border/10"><LanguageSelector /></div></section>
