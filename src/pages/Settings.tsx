...
export default function SettingsPage() {
  ...
  const [username, setUsername] = useState(""); const [exporting, setExporting] = useState(false); const [relinking, setRelinking] = useState(false); const [logoutConfirmOpen, setLogoutConfirmOpen] = useState(false); const [importOpen, setImportOpen] = useState(false); const [subscriptionOpen, setSubscriptionOpen] = useState(false);
  ...
  return <AppLayout><div className="mx-auto max-w-5xl space-y-7 px-4 py-6 sm:px-6 lg:px-10 lg:py-12">
    <section className="space-y-4"><SectionTitle title={t("profile_planUsage")} /><div className="divide-y divide-border/10"><ActionRow icon={SparklesIcon} label={t("profile_continuumSubscription")} description={currentPlan} onClick={() => setSubscriptionOpen(true)} /></div></section>
  ...
  </div><MarkdownImportDialog open={importOpen} onOpenChange={setImportOpen} onImported={() => { refreshUser(); }} />{subscriptionOpen && <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-black/70 p-3 backdrop-blur-md sm:p-6" role="dialog" aria-modal="true" onMouseDown={(event) => { if (event.target === event.currentTarget) setSubscriptionOpen(false); }}><div className="w-full max-w-md" onMouseDown={(event) => event.stopPropagation()}><SubscriptionContent onClose={() => setSubscriptionOpen(false)} /></div></div>}</AppLayout>;
}
