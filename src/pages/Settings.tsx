...
export default function SettingsPage() {
  ...
  </div><MarkdownImportDialog open={importOpen} onOpenChange={setImportOpen} onImported={() => { refreshUser(); }} />{subscriptionOpen && <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-black/70 p-0 backdrop-blur-md" role="dialog" aria-modal="true" onMouseDown={(event) => { if (event.target === event.currentTarget) setSubscriptionOpen(false); }}><div className="w-full max-w-md" onMouseDown={(event) => event.stopPropagation()}><SubscriptionContent onClose={() => setSubscriptionOpen(false)} /></div></div>}</AppLayout>;
}
